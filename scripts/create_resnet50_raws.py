...
def __create_raw_resnet50(img_filepath, mean_rgb, std_rgb, req_bgr_raw, save_uint8):
    img_raw = __get_img_raw(img_filepath)

    snpe_raw = (img_raw - mean_rgb) / std_rgb # HWC
    ...
    snpe_raw = np.transpose(snpe_raw, (2,0,1))
    print(snpe_raw.shape)
    print(snpe_raw)
    img_filepath = os.path.abspath(img_filepath)
...
def __resize_square_to_jpg(src, dst, size,resize_type):
    ...
    # center crop to square    # mean_raw = __create_mean_raw(img_raw, mean_rgb)
...
def convert_img(src,dest,size,resize_type):
    ...
    print("Converting images for resnet50 network.")
    ...
    for root,dirs,files in os.walk(dest):
        for jpgs in files:
            src_image=os.path.join(root, jpgs)
            if('.JPEG' in src_image):
                mean_rgb = np.array([123.675, 116.28, 103.53]).reshape(1, 1, 3)
                std_rgb = np.array([58.395, 57.12, 57.375]).reshape(1, 1, 3)
                __create_raw_resnet50(src_image,mean_rgb,std_rgb,False,False)
